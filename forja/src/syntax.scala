...
object syntax:
  ...
  object ValueContext extends Context:
    ...
    sealed trait NodeSpanApplyImpl[-T] extends NodeApplyImpl[T]:
      final def attrs(arg: T, builder: mutable.Growable[(Token, Node.NodeImpl)]): Unit = ()
    ...
    given nodeSpanApplyImplNodeIter: [T <: IterableOnce[Node]] => NodeSpanApplyImpl[IterableOnce[Node]]:
      def children(arg: IterableOnce[Node], builder: mutable.Growable[Node.NodeImpl]): Unit =
        builder ++= arg.iterator.map(_.impl)
      end children
    end nodeSpanApplyImplNodeIter

    given nodeApplyImplAttr: NodeApplyImpl[(Token, Node)]:
      def children(arg: (Token, Node), builder: mutable.Growable[Node.NodeImpl]): Unit = ()
      def attrs(arg: (Token, Node), builder: mutable.Growable[(Token, Node.NodeImpl)]): Unit =
        builder += ((arg._1, arg._2.impl))
      end attrs
    end nodeApplyImplAttr

    given nodeSpanApplyImplEmbed: [T <: Matchable : Node.Embed] => NodeSpanApplyImpl[T]:
      def children(arg: T, builder: mutable.Growable[Node.NodeImpl]): Unit =
        builder += Node.embed(arg).impl
      end children
    end nodeSpanApplyImplEmbed

    given nodeApply: [Rest <: Tuple] => (impls: TupleOf[Tuple.Map[Rest, NodeApplyImpl]]) => NodeApply[Token *: Rest, Node]:
      def apply(arg: Token *: Rest): Node =
        ...
        (arg.productIterator.drop(1).zip(impls.value.productIterator)).foreach: (arg, impl) =>
    ...
    given nodeSpanApply: [T <: Tuple] => (impls: TupleOf[Tuple.Map[T, NodeSpanApplyImpl]]) => NodeSpanApply[T, Seq[Node]]:
      def apply(arg: T): Seq[Node] =
        val builder = Seq.newBuilder[Node.NodeImpl]
        (arg.productIterator.zip(impls.value.productIterator)).foreach: (arg, impl) =>
          impl.asInstanceOf[NodeSpanApplyImpl[Any]].children(arg, builder)
        
        builder.result().map: impl =>
          new Node(
            impl = impl,
            nodeParentInfo = Node.NodeParentInfo.Orphan,
          )
  ...
  object PatternContext extends Context:
    def lit[T <: Matchable: Node.Embed](value: T, values: T*): Pattern[T] =
      values.foldLeft(new Pattern.EmbedLiteralPattern(value): Pattern[T]): (acc, v) =>
        acc | new Pattern.EmbedLiteralPattern(v)
    end lit
    ...
    given nodeSpanApplyIncludeNonTuple: [T, Rest <: Tuple, RestU <: Tuple] => NotGiven[T <:< Tuple] => (impl: NodeSpanApplyImpl[Rest, RestU]) => NodeSpanApplyImpl[Pattern.Include[T] *: Rest, T *: RestU]:
    ...
    given nodeSpanApplyIncludeTuple: [T <: Tuple, Rest <: Tuple, RestU <: Tuple] => (includedSize: ValueOf[Tuple.Size[T]]) => (impl: NodeSpanApplyImpl[Rest, RestU]) => NodeSpanApplyImpl[Pattern.Include[T] *: Rest, Tuple.Concat[T, RestU]]:
    ...
    given nodeSpanApplySkip: [T, Rest <: Tuple, RestU <: Tuple] => (impl: NodeSpanApplyImpl[Rest, RestU]) => NodeSpanApplyImpl[Pattern[T] *: Rest, RestU]:
    ...
    given `nodeSpanApply...`: [T, Rest <: Tuple, RestU <: Tuple] =>  (impl: NodeSpanApplyImpl[Rest, RestU]) => NodeSpanApplyImpl[`...`.type *: Rest, RestU]:
      def tupleArity: Int = impl.tupleArity
    ...
    given nodeSpanApply: [T <: Tuple, U <: Tuple] => (impl: NodeSpanApplyImpl[T, U]) => NodeSpanApply[T, Pattern[U]]:
      def apply(arg: T): Pattern[U] =
        impl(total = false, arg = arg)
      end apply
    end nodeSpanApply

    given nodeApplyToken: [Rest <: Tuple, U <: Tuple] => (impl: NodeSpanApplyImpl[Rest, U]) => NodeApply[Token *: Rest, Pattern[U]]:
      def apply(arg: Token *: Rest): Pattern[U] = 
        Pattern.tokenExact(arg.head, impl(total = true, arg = arg.tail))
      end apply
    end nodeApplyToken

    given nodeApplyAny: [T <: Tuple, U <: Tuple] => NotGiven[Tuple.Head[T] <:< Token] => (impl: NodeSpanApplyImpl[T, U]) => NodeApply[T, Pattern[U]]:
      def apply(arg: T): Pattern[U] = 

...
sealed trait Query[+T]:
  ...
  final def here[TT >: T, U](at: Query[U])(using ev: TT <:< Node): Query[U] = new here(query.cast(using ev), at)
...
object Query:
  ...
  private final class map[T, U](val query: Query[T], fn: T => U) extends Query[U]:
    protected def runQueryImpl(node: Node): Eval[Option[U]] =
      Eval.defer(query.runQuery(node)).map(_.map(fn))
    end runQueryImpl
  end map

  private final class ap[A, B](val ff: Query[A => B], val fa: Query[A]) extends Query[B]:
    protected def runQueryImpl(node: Node): Eval[Option[B]] =
      Eval.defer(ff.runQuery(node)).flatMap:
  ...
  private final class here[T](val place: Query[Node], val query: Query[T]) extends Query[T]:
    protected def runQueryImpl(node: Node): Eval[Option[T]] =
      place.runQuery(node).flatMap:
        case Some(node) =>
          query.runQuery(node)
        case None => Eval.now(None)
    end runQueryImpl
  ...
  private final class alt[T](val left: Query[T], val right: Query[T]) extends Query[T]:
    import DecisionTree.*
  ...
  private[forja] object DecisionTree:
    private def scanPattern[T, Q[_] <: Query[?] | Pattern[?]](buf: RecordList[T, Q], pattern: Pattern[?]): Chain[Either[RecordList[T, Q], RecordList[T, Q]]] =
      pattern match
        case pattern: Pattern.Tupled[?] => 
          pattern.elems.foldLeft(Chain.one(Right(buf): Either[RecordList[T, Q], RecordList[T, Q]])): (acc, elem) =>
            acc.flatMap:
    ...
    private def scanQuery[T, Q[_] <: Query[?] | Pattern[?]](buf: RecordList[T, Q], query: Query[?]): Chain[Either[RecordList[T, Q], RecordList[T, Q]]] =
      query match
        case Query.empty | (_: pure[?]) | Query.leftSibling | Query.rightSibling | Query.parent | (_: NodeAccessorQuery[?]) =>
          Chain.one(Right(buf))
        case query: on[?] =>
        ...
        case query: ap[?, ?] =>
          ...
          then Chain.one(Left(buf))
            ...
            else if hasLeft then right.map(_.forceLeft) else right
        case query: here[?] =>
    ...
    type RecordList[T, Q[_] <: Query[?] | Pattern[?]] = mutable.ListBuffer[Record[T, Q]]
    type FrozenRecordList[T, Q[_] <: Query[?] | Pattern[?]] = List[FrozenRecord[T, Q]]
    type Record[T, Q[_] <: Query[?] | Pattern[?]] = Q[T] | Branch[T, Q]
    final case class Branch[T, Q[_] <: Query[?] | Pattern[?]](map: mutable.HashMap[Token | EndOfFieldsMarker, RecordList[T, Q]]):
    ...
    extension [T, Q[_] <: Query[?] | Pattern[?]](recordList: RecordList[T, Q])
      def ensureBranch: Branch[T, Q] =
        recordList
          ...
          .getOrElse:
            val branch = new Branch[T, Q](mutable.HashMap.empty[Token | EndOfFieldsMarker, RecordList[T, Q]])
            recordList += branch
            branch
      end ensureBranch
    end extension

    extension [T, Q[_] <: Query[?] | Pattern[?]](either: Either[RecordList[T, Q], RecordList[T, Q]])
    ...
    extension [T, Q[_] <: Query[?] | Pattern[?]](record: Record[T, Q])(using TypeTest[Record[T, Q], Q[T]])
    ...
    type FrozenRecord[T, Q[_] <: Query[?] | Pattern[?]] = Q[T] | FrozenBranch[T, Q]
    final case class FrozenBranch[T, Q[_] <: Query[?] | Pattern[?]](map: Map[Token | EndOfFieldsMarker, FrozenRecordList[T, Q]])

    extension [T, Q[_] <: Query[?] | Pattern[?]](recs: FrozenRecordList[T, Q])(using TypeTest[FrozenRecord[T, Q], Q[T]])
      def run[U](nodeSpan: NodeSpan, runFn: Q[T] => Eval[Option[U]]): Eval[Option[U]] =
        def impl(nodeSpan: NodeSpan, recs: FrozenRecordList[T, Q]): Eval[Option[U]] =
  ...
  final class leftSibling[T](query: Query[T]) extends NodeAccessorQuery[T](query):
  ...
  final class rightSibling[T](query: Query[T]) extends NodeAccessorQuery[T](query):
  ...
  final class rewriteAll(rewrites: on[?]#rewrite*) extends Query[Node]:
    ...
    protected def runQueryImpl(node: Node): Eval[Option[Node]] =
      def impl(nodeSpan: NodeSpan, madeChangesThisIteration: Boolean): Eval[NodeSpan] =
        Eval.defer(rewritesAgg.runPattern(nodeSpan)).flatMap:
          case None =>
            ...
            def firstAvailableParentsSiblingOrRoot(nodeSpan: NodeSpan): NodeSpan =
              nodeSpan.parentOption match
                case None => nodeSpan
                case Some(parent) =>
                  parent.rightSiblingOption match
                    case None => firstAvailableParentsSiblingOrRoot(parent.emptyNodeSpanHere)
                    case Some(sibling) => sibling.emptyNodeSpanHere
            end firstAvailableParentsSiblingOrRoot

            val nextSpan = firstChild.getOrElse(firstAvailableParentsSiblingOrRoot(nodeSpan))
            ...
            then
              ...
              impl(nextSpan, madeChangesThisIteration)
          case Some(((), nodeSpan)) =>
            impl(nodeSpan.take(0), madeChangesThisIteration = true)
      end impl

      impl(node.asOrphan.emptyNodeSpanHere, madeChangesThisIteration = false).map: nodeSpan =>
        val replacementNode =
          nodeSpan
            .headOption

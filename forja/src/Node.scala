...
import scala.quoted.quotes
import forja.util.FastPatchTree
import scala.compiletime.asMatchable
import scala.quoted.Type
import scala.util.NotGiven

final class Node @publicInBinary private (private val impl: Node.NodeImpl, private val nodeParentInfo: NodeParentInfo):
  // Regular nodeParentInfo may not refer to a parent that refers back to impl.
  // If we want a parent ref that is up to date, this will lazily populate such a thing.
  // Important benefit of doing it like this: this lazy val is internal and does not invoke any other lazy vals,
  // so it will always expand just 1 parent node. Making nodeParentInfo itself lazy could lead to arbitrary recursion.
  private lazy val nodeParentInfoStable =
    nodeParentInfo match
      case NodeParentInfo.IndexParent(parent, index) =>
        ...
        then nodeParentInfo
        else NodeParentInfo.IndexParent(
          new Node(
            impl = parentImpl.copy(children = parentImpl.children.updated(index, this.impl)),
      ...
      case NodeParentInfo.AttrParent(parent, attr) =>
        ...
        then nodeParentInfo
        else NodeParentInfo.AttrParent(
          new Node(
            impl = parentImpl.copy(attrs = parentImpl.attrs.updated(attr, this.impl)),
            nodeParentInfo = parent.nodeParentInfo,
          ),
          attr,
        )
  ...
  def tokenOption: Option[Token] =
    impl match
      ...
      case _: NodeImpl.EmbedNode => None
  end tokenOption

  def valueOption[T](using embed: Embed[T]): Option[T] =
    impl match
      case _: NodeImpl.TokenNode => None
...
object Node:
  ...
  transparent inline def apply(inline args: PatternApplyArg*)(using inline ctx: PatternContext): Pattern[Tuple] =
    ${ applyImplPattern('args) }
  end apply

  type NodeApplyArg = Node | (Token, Node)

  transparent inline def apply(inline token: Token, inline args: NodeApplyArg*)(using NotGiven[PatternContext]): Node =
    ${ applyImplNode('token, 'args) }
  end apply

  private def applyImplPattern(argsExpr: Expr[Seq[PatternApplyArg]])(using Quotes): Expr[Pattern[Tuple]] =
    argsExpr match
      case Varargs(argExprs) =>
        argExprs match
          case Seq('{ $token : Token }, rest*) =>
            '{ Pattern.tokenExact($token, NodeSpan($argsExpr*)(using PatternContext)) }
          case _ =>
            '{ Pattern.tokenAny(NodeSpan($argsExpr*)(using PatternContext)) }
  end applyImplPattern

  private def applyImplNode(tokenExpr: Expr[Token], argsExpr: Expr[Seq[NodeApplyArg]])(using Quotes): Expr[Node] =
    argsExpr match
      case Varargs(argExprs) =>
      ...
      case Seq(token, _*) =>
        quotes.reflect.report.errorAndAbort("first argument must be a token", tokenExpr)
      case Seq() =>
        quotes.reflect.report.errorAndAbort("Node requires at least one argument")
  end applyImplNode
  ...
  private enum NodeImpl:
    this match
      ...
      case EmbedNode(value) =>
        assert(Embed.embedByClass.contains(value.getClass()), s"!!internal error: using unregistered embed of ${value.getClass()}")

    case TokenNode(token: Token, children: FastPatchTree[NodeImpl], attrs: Map[Token, NodeImpl])
    case EmbedNode(value: Any)
  end NodeImpl

  final class NodeChildren private[Node] (parent: Node) extends IndexedSeq[Node]:
    private[Node] val implSeq: FastPatchTree[NodeImpl] = parent.impl match
      case NodeImpl.TokenNode(token, children, attrs) => children
      case _: NodeImpl.EmbedNode => FastPatchTree.empty
    end implSeq
  ...
  final class NodeAttrs private[Node] (parent: Node) extends Map[Token, Node]:
    private[Node] val implMap: Map[Token, NodeImpl] = parent.impl match
      ...
      case _: NodeImpl.EmbedNode => Map.empty
    end implMap
  ...
  object NodeSpan:
    ...
    transparent inline def apply(inline args: PatternApplyArg*)(using PatternContext): Pattern[Tuple] =
      ${ applyImplPattern('args) }
    end apply
        ...
        case Varargs(argExprs) =>
          ...
          Expr.ofTupleFromSeq(includes) match
            case '{ $_ : includesTuple } =>
              '{ Pattern.Tupled($argsExpr*).asInstanceOf[Pattern[Tuple & includesTuple]] }
    end applyImplPattern
  end NodeSpan

  private final class SingletonNodeSpan(node: Node, includesMe: Boolean) extends NodeSpan:
    def apply(i: Int): Node =
      if includesMe && i == 0
      then node
    ...
    def replaceThis(nodes: Iterable[Node]): NodeSpan =
      ...
      require(nodes.size == 1, s"tried to make an orphaned span have a number of nodes other than 1 (${nodes.size})")
  ...
  private final class IndexedParentNodeSpan(parent: Node, start: Int, val length: Int) extends NodeSpan:
    def apply(i: Int): Node =
      parent
        .children
        .view
    ...
    def replaceThis(nodes: Iterable[Node]): NodeSpan =
      ...
      IndexedParentNodeSpan(
        parent = new Node(
          impl = NodeImpl.TokenNode(
            token = token,
            children = parent.children.implSeq.patch(start, nodes.iterator.map(_.impl), length),
